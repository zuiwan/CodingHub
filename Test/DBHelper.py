...
SQLALCHEMY_DATABASE_URI = 'mysql://root:RussellCloud2017@139.224.114.10:3306/Russell'
# 导入:
...
def connect_test():
    ...
    conn = mysql.connector.connect(host="139.224.114.10", user='root', password='RussellCloud2017', database='Russell', use_unicode=True)
    cursor = conn.cursor()
...
def connect_test_sqlalchemy():
    # connect test passed
    # 创建对象的基类:
    Base = declarative_base()
    # 初始化数据库连接:
    engine = create_engine('mysql+mysqlconnector://root:RussellCloud2017@139.224.114.10:3306/Russell')
    # 创建DBSession类型:
...
#############  No SQLAlchemy #########
init_db_config = {}
init_db_config['host'] = "test.dl.russellcloud.com"
init_db_config['db'] = "Russell"
init_db_config['port'] = 3306
init_db_config['user'] = "root"
init_db_config['passwd'] = "RussellCloud2017"
init_db_config['name'] = "default"

logger = LogCenter.instance().get_logger('DBHelper')
...
class Test_Data_Manager:
    ...
    def is_owned_by_username(self, model_table, col, username):
        # type: (str, dict, str) -> bool
        obj_sql = DBModel.sql_select(model_table,
                                 keys=['owner_id'],
                                 where=DBModel.sql_and(col))
        return self.db_model.GetOne(obj_sql).get('owner_id') == \
               self.get_userid_by_username(username).get('id')

    def get_userid_by_username(self, username):
        sql = DBModel.sql_select("user",
                                 keys=['id'],
                                 where=DBModel.sql_and({'username':username}))
        return self.db_model.GetOne(sql)

    def get_username_by_userid(self):
        pass
    def get_user(self, keys=None):
        pass
    def get_users(self, keys=None, limit=10):
        if keys is None:
            keys = self.default_user_keys
        sql = DBModel.sql_select("user", keys)
        return self.db_model.GetList(sql, options={'limit_count': limit})

    def get_project(self):
        pass
    def get_projects(self, keys=None, limit=10):
        if keys is None:
            keys = self.default_project_keys
        sql = DBModel.sql_select("project", keys)
        return self.db_model.GetList(sql, options={'limit_count': limit})

    def get_project_names(self, limit=10):
        names = self.get_projects(['name'], limit)
        return [name['name'] for name in names]

    def get_user_names(self, limit=10):
        names = self.get_users(["username"], limit)
        return [name['username'] for name in names]

    def get_one_project_by_user(self, user_id):
        sql = DBModel.sql_select("project",
                                 keys=self.default_project_keys,
                                 where=DBModel.sql_and({'owner_id': user_id}))
        return self.db_model.GetOne(sql)
